...
--------------------
Reading order : 1 of 20   (foundations)
Duty          : Raw file access: PLY meshes and landmark CSVs off disk.

The only module that knows the filesystem layout. Landmark CSVs store each
row as a bracketed string like '0,[x y z]', parsed by _load_landmarks.
Subject ids come from listing the mesh directory, so the mesh set defines
the cohort.
"""
